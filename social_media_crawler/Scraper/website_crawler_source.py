...
class BaseCrawlerConfig(BaseSourceConfig):
    ...
    @abstractmethod
    def extract_url(self, url: str, url_id: str = ''):
        pass
...
class TrafilaturaCrawlerConfig(BaseCrawlerConfig):
    ...
    def extract_url(self, url: str, url_id: str = ''):
        ...
        url_id = url_id or f'{mmh3.hash(url, signed=False):02x}'
    ...
    def find_urls(self, url: str):
        ...
        urls: list[str] = []
        if self.is_sitemap:
            urls = sitemaps.sitemap_search(
                url=url, target_lang=self.target_language,
            )
        elif self.is_feed:
