...
class YouTubeCommentExtractor(BaseModel):
    ...
    _YT_INITIAL_DATA_REGEX: str = r'(?:window\s*\[\s*["\']ytInitialData["\']\s*\]|ytInitialData)\s*' + \
        '=\s*({.+?})\s*;\s*(?:var\s+meta|</script|\n)'
    ...
    def _fetch_comments(self, until_datetime: Optional[datetime] = None):
        ...
        data = json.loads(
            self._regex_search(
                html, self._YT_INITIAL_DATA_REGEX,
            ),
        )
        ...
        while continuations:
            ...
            if list(self._search_dict(response, 'externalErrorMessage')):
                logger.warning(
                    'Error returned from server: %s', next(
                        self._search_dict(response, 'externalErrorMessage'),
                    ),
                )
                return

            if needs_sorting:
                sub_menu: dict[str, Any] = next(
                    self._search_dict(
                        response, 'sortFilterSubMenuRenderer',
                    ), {},
                )
                sort_menu = sub_menu.get('subMenuItems', [])
                if self.sort_by < len(sort_menu):
                    continuations = [
                        sort_menu[self.sort_by]['serviceEndpoint'],
                    ]
                    needs_sorting = False
                    continue
            ...
            for action in actions:
                for item in action.get('continuationItems', []):
                    if action['targetId'] == 'comments-section':
                        ...
                        continuations[:0] = [
                            ep for ep in self._search_dict(
                    ...
                    if self.fetch_replies:
                        if action['targetId'].startswith('comment-replies-item') and 'continuationItemRenderer' in item:
                            ...
                            continuations.append(
                                next(self._search_dict(item, 'buttonRenderer'))[
                                    'command'
                                ],
                            )
