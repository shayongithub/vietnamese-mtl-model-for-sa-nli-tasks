...
DATETIME_STRING_PATTERNV2 = '%m-%d-%YT%H:%M:%SZ'
    ...
    for key, val in dictionary.items():
        ...
        if isinstance(val, list):
            for sub_dict in val:
                ...
                out.update(
                    {key + separator + key2: val2 for key2, val2 in deeper},
                )
...
def obj_to_markdown(
    obj: Any,
    level: int = 1,
    str_enclose_start: str | None = None,
    str_enclose_end: str | None = None,
) -> str:
    key_prefix = '*' * level
...
def dict_to_object(
    dictionary: dict[str, Any],
    class_name_key: str | None = '_target_',
    ...
    for k, v in dictionary.items():
        if k == class_name_key:
            ...
            new_dict[k] = dict_to_object(
                dictionary=v, class_name_key=class_name_key,
            )
        else:
            new_dict[k] = v
