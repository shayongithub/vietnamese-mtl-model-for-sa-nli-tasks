...
class AppStoreScrapperConfig(BaseSourceConfig):
    ...
    def __init__(self, **data: Any):
        ...
        if self.app_url is not None:
            self.app_id, self.countries, self.app_name = AppStoreScrapperConfig.parse_app_url(
                self.app_url,
            )
        else:
...
class AppStoreScrapperSource(BaseSource):
    ...
    def lookup(self, config: AppStoreScrapperConfig, **kwargs) -> list[TextPayload]:
        ...
        for country in config.countries:
            ...
            since_id: int | None = country_stat.get('since_id', None)
            last_index = since_id
            state[country] = country_stat

            scrapper = AppStoreReviewsReader(
                country=country, app_id=config.app_id,
            )
